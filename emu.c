...
DWORD GetEA(CPU* cpu, BYTE mod, BYTE rm) {
    ...
    if (mod == 1) offset += (char)Fetch8(cpu);
    else if (mod == 2) offset += Fetch16(cpu);
    ...
    while (cpu->running) {
        ...
        for (int k = 0; k < 10000 && cpu->running; k++) {
            ...
            if (op < 0x40 && (op & 7) < 6) {
                ...
                switch (op & 0x0F) {
                case 0x2: jmp = cpu->fC; break;
                case 0x3: jmp = !cpu->fC; break;
                case 0x4: jmp = cpu->fZ; break;
                case 0x5: jmp = !cpu->fZ; break;
                case 0x6: jmp = cpu->fC || cpu->fZ; break;
                case 0x7: jmp = !cpu->fC && !cpu->fZ; break;
                case 0x8: jmp = cpu->fS; break;
                case 0x9: jmp = !cpu->fS; break;
                case 0xC: jmp = cpu->fS; break;
                case 0xD: jmp = !cpu->fS || cpu->fZ; break;
                case 0xE: jmp = cpu->fS || cpu->fZ; break;
                case 0xF: jmp = !cpu->fS && !cpu->fZ; break;
                }
                ...
                BYTE modrm = Fetch8(cpu); DecodeModRM(cpu, modrm, is16, &pRM, &pReg);
                if (op & 2) { if (is16) *(WORD*)pReg = *(WORD*)pRM; else *(BYTE*)pReg = *(BYTE*)pRM;}
                else { if (is16) *(WORD*)pRM = *(WORD*)pReg; else *(BYTE*)pRM = *(BYTE*)pReg;}
            }
            else if (op == 0x8C || op == 0x8E) {
                BYTE modrm = Fetch8(cpu); DecodeModRM(cpu, modrm, 1, &pRM, &pReg);
                int sreg = (modrm >> 3) & 3;
                ...
                do {
                    ...
                    if (op <= 0xA5) {
                        if (is16) *(WORD*)&memory[d] = *(WORD*)&memory[s]; else memory[d] = memory[s];
                        cpu->si += step; cpu->di += step;
                    }
                    else if (op <= 0xA7) {
                        DWORD d_v = is16 ? *(WORD*)&memory[d] : memory[d], s_v = is16 ? *(WORD*)&memory[s] : memory[s];
                        DoALU(cpu, 7, s_v, d_v, is16);
                        cpu->si += step; cpu->di += step;
                    }
                    else if (op <= 0xAB) {
                        if (is16) *(WORD*)&memory[d] = cpu->ax; else memory[d] = cpu->al;
                        cpu->di += step;
                    }
                    else {
                        if (is16) cpu->ax = *(WORD*)&memory[s]; else cpu->al = memory[s];
                        cpu->si += step;
                    }
                ...
                if (op != 0xE3) cpu->cx--;
                if (op == 0xE0) do_jmp = (cpu->cx != 0 && !cpu->fZ);
                else if (op == 0xE1) do_jmp = (cpu->cx != 0 && cpu->fZ);
                else if (op == 0xE2) do_jmp = (cpu->cx != 0);
